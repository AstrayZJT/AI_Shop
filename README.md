...
- 第五阶段多任务编排与状态机源码学习文档：`docs/第五阶段多任务编排与状态机源码学习文档.md`

## 15. 当前边界与未完成项

这次收口的目标是：把“前后端分离 + 客户端/管理端 + AI 客服 + RAG + 主要电商流程”跑起来，并把没做完的地方明确写出来；当前版本已经适合本地演示、课程作业、继续二开，但还不是生产级电商系统。

### 15.1 AI 相关边界

- `SHOP_AI_ENABLED=false` 时，项目走的是本地兜底聊天模型和本地兜底 embedding，只是为了让业务流程、AI 客服入口、知识库问答链路能本地联调，不代表真实大模型效果
- 要验证真正的 AI 对话质量、真实 embedding、真实向量召回，必须显式设置：
  - `SHOP_AI_ENABLED=true`
  - `OPENAI_API_KEY=...`
- 当前默认配置接的是 DashScope 的 OpenAI 兼容接口，但环境变量名仍然读取 `OPENAI_API_KEY`
...
- 支付是模拟支付，没有接真实支付网关、异步回调、退款回调和对账
- 物流是人工填写物流公司、运单号、物流节点，没有接第三方物流平台
- 库存目前能随下单、取消、退款等流程变化，但没有做生产级的库存锁、并发扣减保护和超卖治理
...
1. 直接退款
   - 管理端点击“直接退款”
   - 或调用 `PATCH /api/admin/orders/{id}/refund-review`
   - 这条路径会直接把订单推进到“已退款”

2. 退货回寄后再退款
   - 管理端先发送退货指引：`POST /api/admin/orders/{id}/after-sales/return-instructions`
   - 客户端再提交回寄物流：`POST /api/orders/{id}/return-shipment`
   - 管理端最后确认收到退货并退款：`POST /api/admin/orders/{id}/after-sales/confirm-return-refund`

如果你要演示“用户回寄 -> 平台确认收货 -> 完成退款”的完整链路，第一步不要走“直接退款”。

### 15.5 仍未补齐的工程化部分

- 没有接短信、邮件、对象存储、搜索引擎、消息队列
- 没有做更细粒度的后台权限模型，当前更偏单体演示管理台
- 自动化测试覆盖还不算完整，当前更依赖本地联调和手工验流程
- 没有继续补完部署脚本、CI/CD、监控告警、生产安全加固

如果后面继续做，建议优先补：真实支付/物流、生产级库存并发控制、自动化测试、部署与监控。
